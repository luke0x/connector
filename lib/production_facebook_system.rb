...
class ProductionFacebookSystem
  ...
  def set_profile(user)
    facebook_execute(user) do |fbsession|
      ...
      fbml = "<div style=\"background-image: url(#{facebook_image_url('/facebook/notification.png')});background-position:left;background-repeat:no-repeat;padding:3px 2px 3px 22px;\">You have <a href=\"http://apps.facebook.com#{FACEBOOK['canvas_path']}\">#{user.current_notifications.size} notifications</a>."
